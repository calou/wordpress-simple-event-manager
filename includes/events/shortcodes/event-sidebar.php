	...
	<aside class="event-sidebar">
		...
		<?php if ( ! empty( $reg_url ) || ! empty( $reg_deadline ) ) : ?>
			...
			if ( ! empty( $reg_deadline ) ) :
				$registration_date   = strtotime( $reg_deadline );
				$registration_closed = $registration_date < time();
				...
				<i class="fas fa-clock"></i>
					<?php
						printf(
							...
							esc_html( date_i18n( get_option( 'date_format' ), $registration_date ) )
						);
					?>
			</div>
				<?php else : ?>
		<div class="event-sidebar-deadline">
				<i class="fas fa-clock"></i>
					<?php
						printf(
							__( 'Registration closed on %s', 'event-manager' ),
							esc_html( date_i18n( get_option( 'date_format' ), $registration_date ) )
						);
					?>
			</div>
				<?php endif; ?>
			<?php endif; ?>
			<?php if ( ! empty( $reg_url ) && ! $registration_closed ) : ?>
